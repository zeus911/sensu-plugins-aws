...
class CheckALBTargetGroupHealth < Sensu::Plugin::Check::CLI
  ...
  def unhealthy_target_groups
    ...
    target_groups.each do |target_group|
      ...
      unhealthy_groups[target_group.target_group_name] = { unhealthy_targets: unhealthy_targets, total_targets: health.target_health_descriptions.size } unless unhealthy_targets.empty?
    end
    unhealthy_groups
  end
